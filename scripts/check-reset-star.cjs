 ...
 try {
  for(const touch of [false,true]) {
   ...
   await page.addInitScript(()=>{
    ...
    window.starFrames=[];window.starPose=null;window.sparkles=0;
    window.requestAnimationFrame=f=>raf.call(window,t=>{
     window.starPose=null;window.sparkles=0;f(t);
   ...
   await page.waitForTimeout(850); // A slow third click must start a new sequence.
   await click(296,36);
   assert.equal(await page.evaluate(code=>localStorage.getItem(code),codes[0]),'win','expired click sequence cannot reset');
   await click(296,36);await click(296,36);
   assert.equal(await page.evaluate(codes=>codes.every(code=>localStorage.getItem(code)===null),codes),true,'third click resets every catalogue entry');
  ...
  console.log('PASS: mouse/touch timed triple-click, spring jiggle, gold particle burst, campaign-only reset, respawn');
