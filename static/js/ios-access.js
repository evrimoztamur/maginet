...
export const request = action => window.webkit.messageHandlers.maginet.postMessage({action});
...
export async function nativeFetch(request) {
    try {
        ...
        failed = true;
        window.webkit.messageHandlers.maginet.postMessage({action:'networkError'});
        return null;
    }
}
// UIKit owns text entry so the keyboard also opens when a canvas action runs in rAF.
if (typeof window !== 'undefined') document.addEventListener('focusin', event => {
    if (event.target.id === 'text-input') {
        window.webkit.messageHandlers.maginet.postMessage({action:'keyboard', value:event.target.value,
